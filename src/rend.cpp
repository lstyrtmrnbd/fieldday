...
rend_description initRend(const rend_type_request& type_request) {
  ...
  auto makeWindow = [](/*type_request*/) -> Window* {
    //depth, stencil, AA, major, minor, profile
    ContextSettings settings(24, 0, 0, 4, 6, ContextSettings::Attribute::Core);
    RenderWindow window = new RenderWindow(VideoMode(1280, 720), "fieldday", Style::Default, settings);
    window.setVerticalSyncEnabled(true);
    window.setKeyRepeatEnabled(false); // dont repeat keypresses on key hold
    window.setActive(true);
    return window;
  };

  auto window = makeWindow(/*type_request*/);
  ...
  auto& [shader, texture, arrays] = describeMyRender(type_request);
  
  //cout << glm::to_string(models[0]) << endl;
  auto rend_me = rend_description {
    window,
    shader,
    texture,
    arrays
...
auto translateInput() {
    ...
    auto reduce_and = [](auto first, auto second) {
      return first && second;
    };

    auto reduce_or = [](auto first, auto second) {
      return first || second;
    };

    auto keydown = [](auto key) -> bool {
      return Keyboard::isKeyPressed(key);
    };
    
    auto andKeysPressed = [](auto... args) -> bool {
      vector keys = { args... };
      return transform_reduce(args);
    };
...
void render(const rend_description& rend_me, bool live) {

  // this stuff should come from state and rend description
  ...
  // take back control and recur as primary loop
  while (window.isOpen()) {
